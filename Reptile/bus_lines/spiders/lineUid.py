...
def get_line_name(file='./data/bus/bus_name.json'):
    ...
    return lst
get_line_name()
#lst = get_line_name('./data/subways/name.json')
# input(lst)
    ...
    def parse(self, response):
        try:
            ...
            for i in range(len(text)):
                for key in text[i].keys():
                    if len(uids) <= 1 and key == 'uid':
                        uids.append(text[i][key])
            yield uidItem
...
#scrapy crawl lineUid -o ./data/bus/uid.json
# 问题：查询关键词过长
# 排除倒数第二条，是地铁数据